...
<body>
    ...
    <script>
            ...
            switch(sortBy) {
                ...
                case 'os-version': // Sort by OS Version (numeric comparison, Ascending: oldest first)
                    return sorted.sort((a, b) => {
                        ...
                        const compareVersionsHelper = (v1, v2) => {
                            v1 = v1 || '0';
                            v2 = v2 || '0';
                            const cleanV1 = v1.split('-')[0].split('.');
                            const cleanV2 = v2.split('-')[0].split('.');
                        ...
                        const aOldestVersion = aHostVersions.sort(compareVersionsHelper)[0];
                        const bOldestVersion = bHostVersions.sort(compareVersionsHelper)[0];
                        
                        return compareVersionsHelper(aOldestVersion, bOldestVersion); 
                    });
        ...
        function renderTenantsOnly(data) {
            ...
            // --- DYNAMIC STATUS CARD FIX ---
            const issuesCard = document.getElementById('issues-card');
            const totalIssues = allHosts.reduce((count, host) => count + (host.issues || []).length, 0);
